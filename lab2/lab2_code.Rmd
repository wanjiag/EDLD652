...
```{r warning=FALSE}
p1 <- ggplot(google_trends_long, aes(x=date, y=score, color=hurricane)) + geom_line()
p1
```
...
```{r warning=FALSE}
p2 <- ggplot(google_trends_long, aes(x=date, y=score, fill=hurricane)) + geom_ribbon(ymin=0, ymax=google_trends_long$score, alpha=0.5, color="white")
p2
```
...
```{r warning=FALSE}
p3 <- ggplot(google_trends_long, aes(x=date, y=hurricane, fill=score)) + geom_tile() + coord_fixed() +
...
landfill$date <- as.Date(landfill$date)

p4 <- p2 + geom_vline(xintercept = landfill$date, color="darkgrey") +
  scale_fill_discrete(name="Hurricane",
                      labels=c("Harvey", "Irma", "Jose", "Maria")) +
  geom_label(data = landfill, aes(x=date, y=max(google_trends_long$score)+10,label = hurricane), fill = "darkgrey", colour = "white", show.legend = FALSE) + theme_minimal() + theme(legend.position="top")
...
ggplot(tv_states_long, aes(x=date, y=score, fill=location)) + 
  geom_ribbon(ymin=0, ymax=tv_states_long$score, color="white") +
  scale_fill_manual(values = color_platte) +
  labs(y="Share of sentences", title = "National cable news networks", caption = "Includes Bloomberg, CNBC, CNN, Fox Business, Fox News and MSNBC.\nFiveThirtyEight") + 
  theme_minimal(base_size = 12) + 
  theme(axis.title.x = element_blank(),
        legend.position="none", 
        plot.title = element_text(hjust = 0.5), 
        plot.caption = element_text(hjust = 0)) +
  geom_vline(xintercept = news_data$date, color="lightgrey", linetype="dotted") + 
  geom_label(data=news_data, aes(x=date, y=3.8, label=news), inherit.aes = FALSE, color="grey80",  label.size = NA)+
  geom_text(data=location_text, aes(x,y,label=location), inherit.aes = FALSE, size=4.5) + 
  ylim(0,4)
```
...
```{r warning=FALSE}
gender_prop = comic_characters %>% group_by(sex) %>% count() %>% mutate(percentage = n/sum(.$n))

ggplot(gender_prop, aes(x="", y=percentage, fill=sex)) + geom_bar(stat="identity", width=1) + coord_polar("y", start=0) + theme_void()
```

2. the first appearance of characters across time, separated by different gender.

```{r warning=FALSE}
gender_prop_year = comic_characters %>% group_by(sex, year) %>% count()

ggplot(gender_prop_year, aes(x=year, y=n, group=sex, color=sex)) + geom_line() 
```
...
```{r warning=FALSE}
gender_prop_month = comic_characters %>% group_by(sex, date) %>% count()

ggplot(gender_prop_month, aes(x=date, y=n, group=sex, color=sex)) + stat_ecdf(geom = "step")
```
...
gender_prop_year$sex = factor(gender_prop_year$sex, levels=c("Male", "Female", "Agender","Genderless","Genderfluid","Transgender"))
...
ggplot(gender_prop_year_no_na, aes(x=year, y=n, fill=sex)) + 
  geom_ribbon(ymin=0, ymax=log(gender_prop_year_no_na$n,2)) + 
