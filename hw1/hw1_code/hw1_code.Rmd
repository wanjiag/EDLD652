...
for (i in 1:length(files)) assign(stringr::str_split(files[i], "/")[[1]][length], read_csv(files[i]))
```
...
df1 = courses.csv %>% right_join(studentInfo.csv) %>% group_by(gender, region, id_student) %>% summarise(m = mean(module_presentation_length)) %>% group_by(gender, region) %>% summarise(m = mean(m))

ggplot(df1, aes(gender, region, fill=m)) + geom_tile() + coord_fixed() + viridis::scale_fill_viridis()
...
```{r}
TMA.assessments.id = assessments.csv %>% select(id_assessment, assessment_type, code_module, code_presentation) %>% filter(assessment_type == "TMA") 
TMA.score = TMA.assessments.id %>% left_join(studentAssessment.csv)

studentInfo.csv %>%  group_by(code_module, code_presentation, id_student) %>% count() %>% filter(n>1)

TMA.student.info = TMA.score %>% inner_join(studentInfo.csv) 
...
df2 = TMA.student.info %>% group_by(age_band) %>% summarise(m_credits = mean(studied_credits, na.rm = TRUE), m_score = mean(score, na.rm = TRUE)) %>% gather("type", "score", -age_band)


ggplot(df2, aes(x=age_band, y=score, color=type)) + 
  geom_point() + geom_line(aes(group=type)) + 
  scale_color_brewer(palette = "Set1", name="Type", labels=c("Mean Credits", "Mean Scores")) + 
  labs(x="Age", y="Scores")+
  theme_minimal()

```

With higer age group, people tend to take less credits but have higher TMS scores.  

### Plot3
```{r}
students.no.TMA.score = studentInfo.csv %>% 
  anti_join(TMA.score, by="id_student") %>% # remove all students with TMA scores
  group_by(gender, region, id_student) %>% count() %>% # avoid counting one student repeatedly
  group_by(gender, region) %>% count() 

ggplot(students.no.TMA.score, aes(x=region, y=nn, fill=gender)) + 
  geom_bar(stat="identity", position =  position_dodge()) + 
  coord_flip() + theme_classic() + scale_fill_brewer(palette = "Dark2") + 
...
```{r}
avg.TMA = TMA.student.info %>% group_by(gender, highest_education, final_result) %>% summarise(mean = mean(score, na.rm=TRUE)) 
```
